...
appendChild(document.querySelector("#HeroImgContainer"), playBtn);
...
// Variables game
let clickedCards: HTMLImageElement[] = [];
let matchedCards = 0;

// Container
const container = document.querySelector<HTMLDivElement>("#contentContainer");
const totalMatches: number = imgUrls.length;
const { progressContainer, progress } = makeProgressBar(totalMatches);

// append before the game board
appendChild(container?.parentElement ?? document.body, progressContainer);
...
imgUrls.forEach((url, index) => {
  ...
  cardImg.addEventListener("click", () => {
    if (clickedCards.length === 2 || cardImg.src.includes(url)) return;
...
function showWinScreen() {
  ...
  // Title
  const title = makeHeader("🏆 You Won", {
    classList: "win-title",
  });
